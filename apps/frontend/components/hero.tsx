...
export function Hero({ backgroundSrc }: HeroProps) {
  return (
    <section className="relative h-screen max-h-screen overflow-hidden">
      ...
      {/* Контент */}
      <div
        className="relative flex flex-col items-center text-center text-accentRed gap-14 bg-white bg-opacity-70 rounded-24 p-[15px] h-fit w-fit mx-auto "
        style={{ marginTop: 'calc(50vh - 200px)' }}
      >
        <h1
          className="font-cormorant text-[128px] tracking-wide underline underline-offset-8 decoration-4 leading-[100px]"
          style={{ textShadow: '3px 0px 3px rgba(0, 0, 0, 0.25)' }}
        >
          FROST HOME
        </h1>
        <p className="text-40 font-cormorant mt-12">Место, куда хочется возвращаться</p>
      </div>
      <a
