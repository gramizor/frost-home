...
export const mainPageBlocksData: SectionElement[] = [
  ...
  },
  {
    ...
    subtitle:
      'Уютные дома с камином, кухней и террасой, купель под звёздами, большая благоустроенная территория для прогулок с дорожками и мягкой подсветкой. Панорамные окна открывают вид на лес и просторную поляну, а мягкие кресла приглашают укутаться в плед с книгой или чашкой чая. \n' +
      'Каждая деталь –  от свечей до настольных игр, создана, чтобы вы почувствовали: здесь вас ждали.',
  ...
  },
  {
    ...
    subtitle:
      ...
      'Для удобства есть безлимитный скоростной интернет, подходящий для удалённой работы. Также из каждого крана в доме течет природная колодезная вода, прошедшая лабораторные исследования и признанная безопасной для питья.',
    imageSrc: thirdSection,
    imageAlt: 'third-section'
  }
]
  ...
  return (
    <div className="flex flex-col gap-[190px] mt-[190px]">
      <div className="flex flex-col mx-auto text-center max-w-7xl gap-[20px] ">
        <span className=" font-cormorant text-64 text-browny italic font-extralight">{title}</span>
        <div className="h-1 bg-accentRed w-full" />
        <span className="font-cormorant text-36 font-extralight px-12 whitespace-pre-line text-browny">
          {subtitle}
        </span>
      </div>
      <ImageCarousel
